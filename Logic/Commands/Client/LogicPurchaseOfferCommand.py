...
class LogicPurchaseOfferCommand(BSMessageReader):
    ...
    def process(self):
        ...
        if id1 == 19:
            self.player.UnlockedPins[str(skin1)]=1
            DataBase.replaceValue(self, 'UnlockedPins', self.player.UnlockedPins)
            ID1 = 8 
            Shop.UpdateOfferData(self, self.offer_index)
        if id2 == 19:
            self.player.UnlockedPins[str(skin2)]=1
            DataBase.replaceValue(self, 'UnlockedPins', self.player.UnlockedPins)
            ID2 = 8 
            Shop.UpdateOfferData(self, self.offer_index)
        if id3 == 19:
            self.player.UnlockedPins[str(skin3)]=1
            DataBase.replaceValue(self, 'UnlockedPins', self.player.UnlockedPins)
            ID3 = 8 
            Shop.UpdateOfferData(self, self.offer_index)
            
        if id1 == 5:
        	self.player.StarPowerUnlocked[str(skin1)]+=1
        	DataBase.replaceValue(self, 'StarPowerUnlocked', self.player.StarPowerUnlocked)
        	ID1 = 3
        	Shop.UpdateOfferData(self, self.offer_index)
        if id2 == 5:
            self.player.StarPowerUnlocked[str(skin2)]+=1
            DataBase.replaceValue(self, 'StarPowerUnlocked', self.player.StarPowerUnlocked)
            ID2 = 3
        if id3 == 5:
            self.player.StarPowerUnlocked[str(skin3)]+=1
            DataBase.replaceValue(self, 'StarPowerUnlocked', self.player.StarPowerUnlocked)
            ID3 = 3

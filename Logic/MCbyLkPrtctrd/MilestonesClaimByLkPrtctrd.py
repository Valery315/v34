...
class MilestonesClaimByLkPrtctrd(BSMessageReader):
    ...
    def process(self):
        if self.Road==10:
            ...
            if self.Season in [4]:
                if self.Level in [2,4,8,10,14,23,34,46]: #золото бп+
                    amount = Help().GetAmountOfLevel(self.Level+1, [3,5,9,11,15,24,35,46], [100,100,100,100,200,200,200,300])
                    Supply(self.client, self.player, "BPLkPrtctrd", {"Type":7,"Amount": amount, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [3,7,11,13,17,20,25,26,31,32,36,40,44]: #биг бокс бп+
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [1,9,19,29,42]: #мега бп+
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 11, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [5]:
                    boxalternate = 12
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [15]:
                    brawler = Help().GetAmountOfLevel(self.Season+1, [5], [44])
                    ...
                    brawler = Help().GetAmountOfLevel(self.Season+1, [5], [18]) #Бравлер Айди (брок)
                    skin = Help().GetAmountOfLevel(self.Season+1, [5], [248]) #Скин айди (суперрейнджер)
                    if self.player.UnlockedSkins[str(skin)] == 0:
                        ...
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [51]:
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                    self.player.BPTOKEN -= 500
                    DataBase.replaceValue(self,'BPTOKEN', self.player.BPTOKEN)
                elif self.Level in [50]:
                    brawler = Help().GetAmountOfLevel(self.Season+1, [5], [44]) #Бравлер Айди (вольт)
                    skin = Help().GetAmountOfLevel(self.Season+1, [5], [244]) #Скин айди (паладин)
                    if self.player.UnlockedSkins[str(skin)] == 0:
                        amount = 1
                        self.Skin = [29,skin]
                        self.Character = [16,brawler]
                        ...
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": 12, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [6,12,22,28,38,48]: #очки силы бп+
                    try:
                        self.Character
                        ...
                        Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
                elif self.Level in [16,18,21,24,27,30,33,35,37,39,41,43,45,47,49]: #пины литл бокс
                    boxalternate = 10
                    Supply(self.client, self.player, "BOXLkPrtctrd", {"BoxDID": boxalternate, "Road": self.Road, "Season": self.Season, "Level": self.Level}).send()
        ...
        import json
        if self.Road in [9] and self.Season in [4]:
            try:
        ...
        if self.Road in [10] and self.Season in [4]:
            try:

...
class MilestonesClaimSupplyByLkPrtctrd(Writer):
    ...
    EXCLUDED_BRAWLER_ID = [0, 1, 2, 3, 7, 8, 9, 14, 22, 27, 30, 45]  # Define the IDs of the brawlers to exclude
    ...
    # Легендарные бравлеры (1-2% от общего числа)
        ...
        if self.id1 == "BPLkPrtctrd":
            ...
            if self.mult1["Type"] == 6:
                ...
                starpower = self.mult1["StarPower"][1]
            ...
            if self.mult1["Type"] == 6:
                ...
                rewardcount = 2 if self.player.StarPowerUnlocked.get(str(self.mult1['StarPower'][1]), 0) == 0 else 1
            ...
            if self.mult1["Type"] not in [1, 6, 9]:
                ...
                self.writeVint(0)
                 ...
                 if self.player.StarPowerUnlocked.get(str(self.mult1["StarPower"][1]), 0) == 0:
                    self.writeVint(1)
                    self.writeBPScId(23, self.mult1["StarPower"][1])
                    self.writeVint(self.mult1["Type"])
                    self.writeBPScId(0, 0)
                    self.writeBPScId(0, 0)
                    self.writeBPScId(0, 0)
            ...
            if self.mult1["Type"] == 7:
                ...
                self.player.StarPowerUnlocked[str(self.mult1["Skin"][1])] = 1
